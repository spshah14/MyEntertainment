...
const Scripted = () => {
    ...
    const fetchData = async () => {
        ...
        const url = `https://api.themoviedb.org/3/discover/tv?api_key=c85a7220743f2e910ce5418be14ce8b8&language=hi-IN&sort_by=popularity.desc&with_type=4&page=5&with_origin_country=IN&include_video=true&append_to_response=videos,images&page=${page}`
    ...
    const fetchMoreData = async () => {
        // setPage(page + 1)
        const url = `https://api.themoviedb.org/3/discover/tv?api_key=c85a7220743f2e910ce5418be14ce8b8&language=hi-IN&sort_by=popularity.desc&with_type=4&page=5&with_origin_country=IN&include_video=true&append_to_response=videos,images&page=${page + 1}`
    ...
    return (
        <>
            ...
            <div className='container my-2'>
                <div className="container my-3 text-center" style={{ color: "rgb(238 238 238)" }}><h3>Scripted Shows</h3></div>
                        ...
                        <div className="row row-cols-2  row-cols-lg-6 g-2 g-lg-3">
                            ...
                            {data.map((element) => {
                                return <div className="col my-3" key={element.id} onClick={() => fetchItems(element.id)} >
                                    <div className="card bg-image hover-overlay mx-2 bcolor moviecard h-100" onClick={() => setScrollableModal(!scrollableModal)} >
                                        {
                                            (element.poster_path !== null)
                                                ...
                                                : ((element.backdrop_path !== null) ? <img src={`${image_path}${element.backdrop_path}`} alt="" style={{ height: '14rem', width: 'auto' }} />
                                                    : <div className='container text-center text-wrap' style={{ height: '14rem', width: 'auto', display: "flex", justifyContent: 'center', alignItems: 'center' }}><b>{element.name}</b></div>)
                                        }
                                        <a href='#!'>
